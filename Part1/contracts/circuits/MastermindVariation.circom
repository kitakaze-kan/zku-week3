...
template MastermindVariation() {
    ...
    // Create a constraint that the solution and guess digits are all less than 25.
    for (j=0; j<3; j++) {
        lessThan[j] = LessThan(32);
        lessThan[j].in[0] <== guess[j];
        lessThan[j].in[1] <== 25;
        lessThan[j].out === 1;
        lessThan[j+3] = LessThan(32);
